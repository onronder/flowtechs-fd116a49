...
export async function fetchShopifySchema(
  ...
  if (!config.storeName || !config.accessToken || !config.apiSecret) {
    return errorResponse("Missing required Shopify credentials: store name, access token, or API secret");
  }
